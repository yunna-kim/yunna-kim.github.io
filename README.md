...
## v8 updates
- Added Kyung Hee Road YouTube and Brunch interview links to Media.
- Added detailed periods for academic/professional service.
- Added public/university activities and clinical therapeutic modalities in Profile.


## v9 updates
- Added Kyung Hee Road YouTube and interview link to media data.
- Added service periods and public/university activity details.
- Added secondary Gmail contact.
- Expanded Profile clinical modalities: herbal medicine, acupuncture, Chuna, EEG, fNIRS, neurofeedback, psychotherapy, psychodrama, meditation, EFT, Brainspotting.


## ORCID 논문 업데이트 안내

홈페이지는 방문자 브라우저에서 ORCID를 실시간으로 읽지 않고, `assets/data/publications.json`에 저장된 정적 데이터를 표시합니다. 따라서 ORCID 페이지에서 Works 수가 늘어도 GitHub Actions의 `Update publications and news` workflow를 수동 실행하거나 예약 실행이 돌아가기 전까지는 홈페이지에 바로 반영되지 않습니다.

GitHub에서 `Actions → Update publications and news → Run workflow`를 누르면 ORCID 기준으로 `assets/data/publications.json`이 갱신됩니다. GitHub 웹 업로드 시 `.github/workflows/update-data.yml` 폴더가 누락되면 자동 업데이트가 작동하지 않습니다.


## v14 update
- Added Books / 저서 page between Publications and Media.
- Translated English Awards and Talks metadata.
